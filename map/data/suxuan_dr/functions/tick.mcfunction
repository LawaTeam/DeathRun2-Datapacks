...
# 跳起来提示
execute as @a[nbt={ActiveEffects:[{Id:8b}]}] at @s run title @s title {"text": "跳起来！","bold": true,"color": "green"}
...
# 教程图加速
execute as @a[x=-276,y=52,z=715,dx=41,dy=6,dz=11] run effect give @s speed 1 2 true
execute as @a[x=-266,y=52,dy=4,z=714,dx=-4,dz=-2] run effect clear @s speed
...
execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:paper"}}] at @s run clear @p
